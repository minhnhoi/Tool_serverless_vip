...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * {
      ...
      body {
        background:
          ...
          radial-gradient(
            ...
            rgba(181, 139, 255, 0.09),
            transparent 60%
          ),
          radial-gradient(
            700px 500px at 50% 130%,
            rgba(255, 90, 196, 0.05),
            transparent 60%
          ),
          var(--bg-deep);
        color: var(--fg);
        font-family:
          "JetBrains Mono", "Cascadia Code", Consolas, Menlo, monospace;
      ...
      .demo-stat.ok .v {
        color: var(--neon-green);
        text-shadow: 0 0 10px rgba(59, 255, 143, 0.35);
      }
      .demo-stat.err .v {
        color: var(--neon-red);
        text-shadow: 0 0 10px rgba(255, 85, 119, 0.4);
      }
      .demo-stat.rate .v {
        color: var(--neon-cyan);
      ...
      .demo-stat.lat .v {
        color: var(--neon-purple);
        text-shadow: 0 0 10px rgba(181, 139, 255, 0.4);
      }
      .demo-stat::after {
        content: "";
        position: absolute;
      ...
      .demo-tail::-webkit-scrollbar {
        width: 8px;
      }
      .demo-tail::-webkit-scrollbar-track {
        background: transparent;
      }
      .demo-tail::-webkit-scrollbar-thumb {
        background: linear-gradient(180deg, #2a2d33, #1a1d22);
        border-radius: 8px;
      }
      .demo-tail::-webkit-scrollbar-thumb:hover {
        background: #3a3d44;
      }
      .demo-row {
        display: grid;
      ...
      .demo-row .blocks i.off {
        opacity: 0.12;
      }
      .demo-row .blocks.cpu i {
        background: #ff8a3d;
      }
      .demo-row .blocks.cpu i.hot {
        background: #ff5577;
      }
      .demo-row .blocks.mem i {
        background: #ffd233;
      }
      .demo-row .blocks.mem i.hot {
        background: #ff8a3d;
      }
      .demo-row .blocks.req i {
        background: #19d2ff;
      }
      .demo-row .blocks.lat i {
        background: #3bff8f;
      }
      .demo-row .blocks.lat i.hot {
        background: #ffd233;
      }
      .demo-row .val {
        color: var(--neon-orange);
      ...
      .panel {
        ...
        box-shadow:
          ...
          0 0 30px rgba(181, 139, 255, 0.05);
        backdrop-filter: blur(6px);
        display: flex;
        flex-direction: column;
        overflow: hidden;
      ...
      .scell.act .n {
        color: var(--neon-green);
        text-shadow: 0 0 12px rgba(59, 255, 143, 0.45);
      }
      .scell.dead .n {
        color: var(--neon-red);
        text-shadow: 0 0 12px rgba(255, 85, 119, 0.45);
      }
      .scell.pau .n {
        color: var(--neon-yellow);
      ...
      @keyframes neonBreath {
        ...
        100% {
          box-shadow:
            0 0 0 1px rgba(255, 255, 255, 0.02) inset,
            0 30px 80px rgba(0, 0, 0, 0.55),
            0 0 22px rgba(0, 232, 255, 0.06),
            0 0 60px rgba(0, 232, 255, 0.04);
        }
        50% {
          box-shadow:
            0 0 0 1px rgba(0, 232, 255, 0.08) inset,
            0 30px 80px rgba(0, 0, 0, 0.55),
            0 0 38px rgba(0, 232, 255, 0.18),
            0 0 90px rgba(181, 139, 255, 0.12);
        }
      }
      .term {
      ...
      .panel {
        animation: neonBreath 5.6s ease-in-out infinite;
      }
      ...
      .term::after {
        ...
        background: linear-gradient(
          to bottom,
          rgba(0, 232, 255, 0) 0%,
          rgba(0, 232, 255, 0.06) 45%,
          rgba(181, 139, 255, 0.1) 55%,
      ...
      @keyframes barBeat {
        ...
        50% {
          box-shadow: 0 0 22px rgba(255, 138, 61, 0.55);
        }
      }
      ...
      .demo-row {
        position: relative;
        animation: demoRowIn 0.25s ease-out;
        will-change: auto;
      }
      @keyframes demoRowIn {
        from {
      ...
      @keyframes drift {
        ...
        100% {
          transform: translate(var(--dx, 0px), calc(-100vh - 40px)) scale(1.2);
          opacity: 0;
        }
      }
  ...
  <body>
        ...
        <div class="powershell">
          <div class="term" id="term">
            ...
            <div class="titlebar">
              ...
              <div class="tab">
                <span class="ico"></span>PowerShell \u2014 Tool Ping Serverless
              </div>
              <div class="crumbs">
                <span class="pulse"></span
                ><b id="ws-state">connecting</b> \u00b7 mirror://process.stdout
              </div>
            </div>
            <div class="hint">
              Live mirror of the backend terminal. Press <b>End</b> to follow
              tail.
            </div>
            <div class="boot-ribbon" id="boot-ribbon">
              <span class="dot"></span>BOOT SEQUENCE \u00b7 INITIALISING
            </div>
            <div class="screen" id="screen">
              <pre class="out" id="out" data-testid="terminal-output"></pre>
            </div>
          </div>
            ...
            <div class="demo-screen">
              ...
              <div class="demo-stats" id="demo-stats" data-testid="demo-stats">
                <div class="demo-stat">
                  ...
                  <div class="v" id="ds-total">0</div>
                </div>
                <div class="demo-stat ok">
                  <div class="k">OK</div>
                  <div class="v" id="ds-ok">0</div>
                </div>
                <div class="demo-stat err">
                  <div class="k">Errors</div>
                  <div class="v" id="ds-err">0</div>
                </div>
                <div class="demo-stat rate">
                  <div class="k">Req / s</div>
                  <div class="v" id="ds-rate">0.0</div>
                </div>
                <div class="demo-stat lat">
                  <div class="k">Avg Lat</div>
                  <div class="v" id="ds-lat">
                    0<span
                      style="
                        font-size: 9px;
                        color: var(--fg-dim);
              ...
              <div class="demo-shutdown" id="demo-shutdown">
                <span class="ts" id="demo-shut-ts"></span
                ><span
                  class="warn"
                  style="color: var(--neon-yellow); font-weight: 700"
                  >\u26a0 WARN</span
                ><span class="tag">(proc)</span> Received SIGINT, shutting
                down\u2026
              </div>
            </div>
          </div>
          ...
          <section class="panel sched" id="panel-sched">
            ...
            <div class="body">
              <div class="sgrid" data-testid="scheduler-summary">
                <div class="scell tot">
                  <div class="n" id="sm-total">0</div>
                  <div class="l">jobs</div>
                </div>
                <div class="scell act">
                  <div class="n" id="sm-active">0</div>
                  <div class="l">active</div>
                </div>
                <div class="scell dead">
                  <div class="n" id="sm-dead">0</div>
                  <div class="l">dead</div>
                </div>
                <div class="scell pau">
                  <div class="n" id="sm-paused">0</div>
              ...
              <div class="strip">
                <div class="kv">
                  ...
                  <div class="v">
                    <span id="kv-tput">0.00</span><small>req/s</small>
                  </div>
                </div>
                <div class="kv">
                  <div class="k">Avg Latency</div>
                  <div class="v">
                    <span id="kv-lat">0</span><small>ms</small>
                  </div>
                </div>
                <div class="kv">
                  <div class="k">Error rate</div>
                  <div class="v">
                    <span id="kv-err">0.0</span><small>%</small>
                  </div>
                </div>
              </div>
          ...
          <section class="panel vit">
            ...
            <div class="head">
              ...
              <span class="sub"
                ><span
                  ...
                  ><span class="pulse"></span>RUNNING</span
                ></span
              >
            </div>
            <div class="body" data-testid="status-bar">
              <div class="vital">
                ...
                <div class="bar" id="bar-cpu"><i></i></div>
                <span class="val" id="val-cpu">--%</span>
              </div>
              <div class="vital">
                <span class="lbl">MEM</span>
                <div class="bar" id="bar-mem"><i></i></div>
                <span class="val" id="val-mem">--%</span>
              </div>
              <div class="vital">
                <span class="lbl">TEMP</span>
                <div class="bar" id="bar-temp"><i></i></div>
                <span class="val" id="val-temp">--\u00b0C</span>
              </div>
              <div class="vital">
                <span class="lbl">UP</span>
                <div class="bar" style="visibility: hidden"><i></i></div>
                <span class="val" id="val-up">--s</span>
              </div>
            </div>
          </section>
        </aside>
    ...
    <script>
      (function fitStage() {
        function apply() {
          window.dispatchEvent(new Event("stage:resized"));
        }
        apply();
      ...
      function applySgr(seq, st) {
        const parts = (seq === "" ? "0" : seq)
          .split(";")
          .map((x) => (x === "" ? 0 : Number(x)));
        for (let k = 0; k < parts.length; k++) {
          const code = parts[k];
          if (code === 0) {
            Object.assign(st, freshStyle());
          } else if (code === 1) {
            st.bold = true;
          } else if (code === 2) {
            st.dim = true;
            ...
            st.color = PALETTE_16[code - 30];
          } else if (code >= 90 && code <= 97) {
            st.color = PALETTE_16[code - 90 + 8];
          } else if (code === 38) {
            const mode = parts[k + 1];
            if (mode === 5) {
              st.color = color256(parts[k + 2] || 0);
              k += 2;
            } else if (mode === 2) {
              const r = parts[k + 2] || 0,
                g = parts[k + 3] || 0,
      ...
      function setBar(id, ratio) {
        ...
        if (r >= 0.8) el.classList.add("crit");
        else if (r >= 0.55) el.classList.add("warn");
      ...
      function ago(ts) {
        ...
        const s = Math.max(
          0,
          Math.round((Date.now() - new Date(ts).getTime()) / 1000),
        );
        if (s < 60) return s + "s";
      ...
      function renderJobs(jobs) {
        ...
        if (!jobs || jobs.length === 0) {
          root.innerHTML =
            '<div class="empty">No scheduled jobs yet.<br><span style="opacity:.6">Add one from the dashboard.</span></div>';
          return;
        }
        JOB_BY_URL.clear();
        ...
        root.innerHTML = sorted
          .map((j) => {
            ...
            const rt = j.lastResponseTime
              ? j.lastResponseTime + "ms"
              : "\u2014";
            return (
              "" +
              '<div class="job ' +
      ...
      function flashJob(url, status) {
        ...
        el.style.background =
          status === "up" ? "rgba(59,255,143,.10)" : "rgba(255,85,119,.12)";
        el.style.boxShadow =
          status === "up"
            ? "inset 0 0 0 1px rgba(59,255,143,.45)"
            : "inset 0 0 0 1px rgba(255,85,119,.45)";
        setTimeout(() => {
          el.style.background = "";
          el.style.boxShadow = "";
        }, 700);
      ...
      function applyLatencySample(sample) {
        ...
        if (LATENCY_HISTORY.length > 60)
          LATENCY_HISTORY = LATENCY_HISTORY.slice(-60);
        ...
        $("feed-rate").textContent =
          (sample.throughput || 0).toFixed(2) + " req/s";
        drawSpark();
      }
      ...
      setInterval(() => {
        $("sched-clock").textContent = new Date().toLocaleTimeString("en-GB", {
          hour12: false,
        });
      }, 1000);
        ...
        try {
          ...
          if (Array.isArray(j.latency)) {
            ...
            if (last) {
              ...
              if (typeof window.__demoApplyLatency === "function")
                window.__demoApplyLatency(last);
            }
            drawSpark();
          }
        } catch (e) {}
      ...
      (function matrixRain() {
        ...
        function frame() {
          ...
          for (let i = 0; i < cols; i++) {
            ...
            cx.fillStyle = "rgba(0,232,255," + (0.18 + 0.35 * t) + ")";
            cx.fillText(ch, x, y);
      ...
      function connect() {
        ...
        ws.addEventListener("message", async (ev) => {
          ...
          if (m.type === "snapshot") {
            ...
            firePacket(p.status);
            if (typeof window.__demoRecordPing === "function")
              window.__demoRecordPing(p);
          } else if (m.type === "latency") {
            applyLatencySample(m.data);
            if (typeof window.__demoApplyLatency === "function")
              window.__demoApplyLatency(m.data);
          }
        });
      }
      ...
      (function demoRuntime() {
        ...
        const seedEvents = [
          () =>
            '<span class="ts">' +
            ts() +
            '</span> <span class="ok">\u2713 OK</span>   <span class="tag">(http)</span> <span class="msg">Server listening on port ' +
            PORT +
            "</span>",
          () =>
            '<span class="ts">' +
            ts() +
            '</span> <span class="ws">\u21b3 ES</span>   <span class="tag">(ws)</span>   <span class="msg">WebSocket bus available at ws://localhost:' +
            PORT +
            "/ws</span>",
          () =>
            '<span class="ts">' +
            ts() +
            '</span> <span class="ok">\u2713 OK</span>   <span class="tag">(ui)</span>   <span class="msg">Live console UI at http://localhost:' +
            PORT +
            "/</span>",
          () =>
            '<span class="ts">' +
            ts() +
            '</span> <span class="cron">\u21ba CRON</span> <span class="tag">(cron)</span> <span class="msg">Per-job ping timers active \u2014 interval driven by frontend setup</span>',
        ];
        seedEvents.forEach((mk, i) =>
          setTimeout(() => pushLog(mk()), 250 + i * 420),
        );
        setInterval(() => {
          ...
          const live = [
            () =>
              '<span class="ts">' +
              ts() +
              '</span> <span class="ok">\u2713 OK</span>   <span class="tag">(http)</span> <span class="msg">GET /api/system/health \u2192 200 ' +
              (4 + Math.floor(Math.random() * 40)) +
              "ms</span>",
            () =>
              '<span class="ts">' +
              ts() +
              '</span> <span class="cron">\u21ba CRON</span> <span class="tag">(cron)</span> <span class="msg">tick \u00b7 next sweep in ' +
              (15 + Math.floor(Math.random() * 120)) +
              "s</span>",
            () =>
              '<span class="ts">' +
              ts() +
              '</span> <span class="ws">\u21b3 ES</span>   <span class="tag">(ws)</span>   <span class="msg">broadcast metrics \u2192 ' +
        ...
        function refreshCounters() {
          const now = Date.now();
          while (PINGS_BUCKET.length && now - PINGS_BUCKET[0].t > 60000)
            PINGS_BUCKET.shift();
        ...
        tailEl.addEventListener("scroll", () => {
          stickToBottom =
            tailEl.scrollTop + tailEl.clientHeight >= tailEl.scrollHeight - 12;
        });
        ...
        function appendTailRow(snap) {
          ...
          setBlocks(
            row.querySelector(".blocks.req"),
            Math.min(1, snap.req / 8),
          );
          setBlocks(
            row.querySelector(".blocks.lat"),
            Math.min(1, snap.lat / 600),
            0.7,
          );
          while (tailEl.children.length > TAIL_CAP)
            tailEl.removeChild(tailEl.firstChild);
        ...
        function tickTail() {
          if (DEMO_SHUTDOWN) return;
          const now = PINGS_BUCKET.length
            ? PINGS_BUCKET[PINGS_BUCKET.length - 1]
            : null;
