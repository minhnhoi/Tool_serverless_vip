...
function maskUrl(url) {
  ...
  const s = String(url);
  if (s.length <= 14)
    return "*".repeat(Math.max(6, Math.floor(s.length / 2))) + s.slice(-6);
...
async function loadActor(req) {
  try {
    ...
    if (u)
      return { id: u._id, name: u.name, username: u.username, role: u.role };
  } catch (_) {}
  return {
    id: req.user.sub,
...
router.get("/stats", async (req, res) => {
  ...
  if (range === "24h") {
    since = new Date(now - 24 * 3600e3);
    bucketMs = 3600e3;
  } else if (range === "7d") {
    since = new Date(now - 7 * 24 * 3600e3);
    bucketMs = 6 * 3600e3;
  } else if (range === "30d") {
    since = new Date(now - 30 * 24 * 3600e3);
    bucketMs = 24 * 3600e3;
  } else {
    since = null;
    bucketMs = 24 * 3600e3;
  ...
  const avgResponseTime =
    total > 0 ? logs.reduce((s, l) => s + (l.responseTime || 0), 0) / total : 0;
...
router.get("/export", async (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    "attachment; filename=cron-monitor-jobs.csv",
  );
  res.send(lines.join("\n"));
});
...
router.post("/import", upload.single("file"), async (req, res) => {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    let parsedHeaders = {};
    try {
      parsedHeaders = JSON.parse(cols[idx("headers")] || "{}");
    } catch {}
...
router.get("/:id", async (req, res) => {
  ...
  const avgResponseTime =
    total > 0
      ? Math.round(logs.reduce((s, l) => s + (l.responseTime || 0), 0) / total)
      : 0;
...
router.put("/:id", async (req, res) => {
  ...
  if (err === "forbidden")
    return res
      .status(403)
      .json({ error: "Bạn không phải người tạo link này." });
  ...
  if (update.interval !== undefined)
    update.interval = Number(update.interval) || 1;
  if (update.expectedStatus !== undefined)
    update.expectedStatus = Number(update.expectedStatus) || 200;
...
router.post("/:id/visibility", async (req, res) => {
  ...
  const next =
    typeof req.body?.linkVisible === "boolean"
